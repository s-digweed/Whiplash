...
# Local IP pattern to strip from image/icon tags
_LOCAL_IP = re.compile(r'^(https?://)?192\.168\.|^(https?://)?10\.|^(https?://)?172\.(1[6-9]|2[0-9]|3[01])\.')
...
def _strip_local_images(prog: ET.Element) -> ET.Element:
    """Remove icon and image tags with local IP addresses from a programme element."""
    for tag in ('icon', 'image'):
        for elem in prog.findall(tag):
            src = elem.get('src') or elem.text or ''
            if _LOCAL_IP.search(src):
                prog.remove(elem)
    return prog
...
def build_epg(wl_root: ET.Element, win_root: ET.Element, biwi_root: ET.Element) -> ET.Element:
    ...
    for source_id, (local_id, display_name, group, source) in CHANNEL_MAP.items():
        ...
        for prog in root.findall("programme"):
            ...
            # Strip local IP images from all channels (Biwi uses local ErsatzTV IPs)
