...
# NOTE: Biwi's ErsatzTV id flips between C6.1.151 and C2.146 — both are mapped.
CHANNEL_MAP = {
...
# Private/local IP addresses to strip from image/icon tags
...
def _strip_local_images(prog: ET.Element) -> ET.Element:
    """Remove icon/image tags pointing at private LAN addresses."""
    for tag in ("icon", "image"):
        for elem in list(prog.findall(tag)):
            src = elem.get("src") or elem.text or ""
            if _LOCAL_IP.search(src.strip()):
                prog.remove(elem)
    return prog
...
def fetch_xml_safe(url: str, name: str) -> ET.Element:
    """Fetch XML, returning an empty <tv> element if fetching/parsing fails."""
    try:
...
def build_epg(wl_root: ET.Element, win_root: ET.Element, biwi_root: ET.Element) -> ET.Element:
    ...
    # Write <channel> blocks first, one per unique local id
    written = set()
    for source_id, (local_id, display_name, group, source) in CHANNEL_MAP.items():
        if local_id in written:
            continue
        written.add(local_id)
    ...
    for source_id, (local_id, display_name, group, source) in CHANNEL_MAP.items():
        ...
        for prog in root.findall("programme"):
            ...
            _strip_local_images(new_prog)
            tv.append(new_prog)
            count += 1
        if count:
            print(f"  {local_id}: {count} programmes (from {source_id})")
